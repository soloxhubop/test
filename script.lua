...
end -- <-- metti qui il tuo ID Roblox numerico
...
-- bgOverlay rimosso (bloccava i click)

-- Usa scala relativa allo schermo per essere piccolo su tutti i dispositivi
local _winW = 320
local _winH = 380
...
if isAdmin() then
    ...
    adminBtn.Size = UDim2.new(1, -40, 0, 26)
    adminBtn.Position = UDim2.new(0, 20, 0, 54)
    ...
    adminBtn.MouseButton1Click:Connect(function()
        ...
        popup.Size = UDim2.new(0, 220, 0, 175)
        popup.Position = UDim2.new(0.5, -110, 0.5, -87)
        popup.BackgroundColor3 = Color3.fromRGB(12, 8, 24)
        popup.BorderSizePixel = 0
        popup.ZIndex = 30
        Instance.new("UICorner", popup).CornerRadius = UDim.new(0, 12)
        local popStroke = Instance.new("UIStroke", popup)
        popStroke.Color = Color3.fromRGB(130, 60, 220)
        popStroke.Thickness = 2
        local popTitle = Instance.new("TextLabel", popup)
        popTitle.Size = UDim2.new(1, -40, 0, 34)
        popTitle.Position = UDim2.new(0, 10, 0, 0)
        popTitle.BackgroundTransparency = 1
        popTitle.Text = "Scegli tipo key"
        popTitle.TextColor3 = Color3.fromRGB(220, 180, 255)
        popTitle.Font = Enum.Font.GothamBold
        popTitle.TextSize = 13
        popTitle.ZIndex = 31
        local closePopBtn = Instance.new("TextButton", popup)
        closePopBtn.Size = UDim2.new(0, 24, 0, 24)
        closePopBtn.Position = UDim2.new(1, -28, 0, 5)
        closePopBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
        closePopBtn.BorderSizePixel = 0
        closePopBtn.Text = "X"
        closePopBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
        closePopBtn.Font = Enum.Font.GothamBold
        closePopBtn.TextSize = 11
        closePopBtn.AutoButtonColor = false
        closePopBtn.ZIndex = 32
        Instance.new("UICorner", closePopBtn).CornerRadius = UDim.new(0.5, 0)
        closePopBtn.MouseButton1Click:Connect(function() popup:Destroy() end)
        local statusLblPop = Instance.new("TextLabel", popup)
        statusLblPop.Size = UDim2.new(1, -20, 0, 20)
        statusLblPop.Position = UDim2.new(0, 10, 0, 150)
        statusLblPop.BackgroundTransparency = 1
        statusLblPop.Text = ""
        statusLblPop.TextColor3 = Color3.fromRGB(100, 255, 150)
        statusLblPop.Font = Enum.Font.Gotham
        statusLblPop.TextSize = 11
        statusLblPop.ZIndex = 31
        local types = {
            {"1 Giorno",    "1D", Color3.fromRGB(180, 60, 60)},
            {"2 Settimane", "2W", Color3.fromRGB(60, 100, 180)},
            {"Lifetime",    "LT", Color3.fromRGB(100, 40, 200)},
        }
        for i, t in ipairs(types) do
            local lbl, keyType, col = t[1], t[2], t[3]
            local btn = Instance.new("TextButton", popup)
            btn.Size = UDim2.new(1, -20, 0, 32)
            btn.Position = UDim2.new(0, 10, 0, 32 + (i-1) * 38)
            ...
            btn.MouseButton1Click:Connect(function()
                local expiry = getExpiryDate(keyType)
                local newKey = generateKey(keyType)
                local fullLine = newKey .. "|" .. expiry
                sendKeyToWebhook("[" .. keyType .. "] " .. fullLine)
                statusLblPop.Text = "Inviata su Discord!"
                btn.Text = "Inviata!"
...
-- tutti gli utenti devono usare la key
local isProtected = false

-- freeze trap rimosso: tutti whitelistati tramite key system
