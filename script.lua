...
local function startAntiRagdoll()
  ...
  Connections.antiRagdoll = RunService.Heartbeat:Connect(function()
    ...
    if st == HST.Physics or st == HST.Ragdoll or st == HST.FallingDown then
      ...
      -- NON azzeriamo la velocity per non bloccare il movimento
    end
...
LocalPlayer.CharacterAdded:Connect(function(char)
  ...
  if SETTINGS.LOCK_ENABLED then stopAutoSwing(); equipBat(); startAutoSwing() end
  if autoTPEnabled then stopRagdollDetector(); startRagdollDetector() end
...
local function executeInternalSteal(prompt)
  ...
  isStealing = true
  task.spawn(function()
    -- fire holdCallbacks immediately
    for _, fn in ipairs(data.holdCallbacks) do task.spawn(fn) end
    -- wait GRAB_DELAY then verify proximity before triggering
    task.wait(SETTINGS.GRAB_DELAY)
    -- proximity check: aspetta max 0.8s che siamo vicini al prompt
    local maxWait = 0.8
    local waited = 0
    local function getPromptPos()
      if not prompt or not prompt.Parent then return nil end
      local par = prompt.Parent
      if par:IsA("Attachment") then return par.WorldPosition
      elseif par:IsA("BasePart") then return par.Position end
      return nil
    end
    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    while waited < maxWait do
      local ppos = getPromptPos()
      if not ppos then break end
      if not hrp or not hrp.Parent then
        hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
      end
      if hrp and (hrp.Position - ppos).Magnitude <= 10 then break end
      task.wait(0.05)
      waited = waited + 0.05
    end
    -- trigger
...
-- prebuild callbacks in background ogni 2s per grab istantaneo
task.spawn(function()
  while true do
    task.wait(2)
...
local function buildMenuPanel()
  ...
  panel.Size = U2(0, 250, 0, 420)
  panel.Position = U2(0.5, -125, 0.5, -210)
  panel.BackgroundColor3 = C3(8, 8, 8)
  panel.BackgroundTransparency = 0.1
  panel.ZIndex = 10
  IN("UICorner", panel).CornerRadius = U1(0, 10)
  AddOutline(panel)
  MakeDraggable(panel, nil)
  -- Tab bar
  local tabBar = IN("Frame", panel)
  tabBar.Size = U2(1, 0, 0, 32)
  tabBar.BackgroundColor3 = C3(18, 18, 18)
  tabBar.BackgroundTransparency = 0
  tabBar.BorderSizePixel = 0; tabBar.ZIndex = 11
  IN("UICorner", tabBar).CornerRadius = U1(0, 10)
  local tabBarFix = IN("Frame", tabBar)
  tabBarFix.Size = U2(1, 0, 0.5, 0); tabBarFix.Position = U2(0,0,0.5,0)
  tabBarFix.BackgroundColor3 = C3(18,18,18); tabBarFix.BorderSizePixel = 0; tabBarFix.ZIndex = 11
  local tabMenuBtn = IN("TextButton", tabBar)
  tabMenuBtn.Size = U2(0, 90, 1, -6); tabMenuBtn.Position = U2(0, 4, 0, 3)
  tabMenuBtn.BackgroundColor3 = C3(40,40,40); tabMenuBtn.Text = "âš™ SETTINGS"
  tabMenuBtn.TextColor3 = C3(255,255,255); tabMenuBtn.Font = FGOTB; tabMenuBtn.TextSize = 10; tabMenuBtn.ZIndex = 12
  IN("UICorner", tabMenuBtn).CornerRadius = U1(0,6)
  local tabKbindBtn = IN("TextButton", tabBar)
  tabKbindBtn.Size = U2(0, 90, 1, -6); tabKbindBtn.Position = U2(0, 98, 0, 3)
  tabKbindBtn.BackgroundColor3 = C3(20,20,20); tabKbindBtn.Text = "âŒ¨ KEYBINDS"
  tabKbindBtn.TextColor3 = C3(130,130,130); tabKbindBtn.Font = FGOTB; tabKbindBtn.TextSize = 10; tabKbindBtn.ZIndex = 12
  IN("UICorner", tabKbindBtn).CornerRadius = U1(0,6)
  local cBtn = IN("TextButton", tabBar)
  cBtn.Size = U2(0,22,0,22); cBtn.Position = U2(1,-26,0,5)
  cBtn.BackgroundColor3 = C3(180,40,40); cBtn.Text = "âœ•"
  cBtn.TextColor3 = C3(255,255,255); cBtn.Font = FGOTB; cBtn.TextSize = 11; cBtn.ZIndex = 12
  IN("UICorner", cBtn).CornerRadius = U1(0,5)
  cBtn.MouseButton1Click:Connect(function()
    menuOpen = false; panelGui:Destroy(); menuPanel = nil
    MenuBtn.Text = "MENU"; MenuBtn.BackgroundTransparency = 0.45
    MenuBtn.TextColor3 = C3(200,200,200)
  end)
  local sep0 = IN("Frame", panel)
  sep0.Size = U2(1,-20,0,1); sep0.Position = U2(0,10,0,33)
  sep0.BackgroundColor3 = C3(45,45,45); sep0.BorderSizePixel = 0; sep0.ZIndex = 11
  -- Scrollframes
  local menuScroll = IN("ScrollingFrame", panel)
  menuScroll.Size = U2(1,0,1,-36); menuScroll.Position = U2(0,0,0,36)
  menuScroll.BackgroundTransparency = 1; menuScroll.ScrollBarThickness = 2
  menuScroll.ScrollBarImageColor3 = C3(80,80,80)
  menuScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; menuScroll.ZIndex = 11
  local kbindScroll = IN("ScrollingFrame", panel)
  kbindScroll.Size = U2(1,0,1,-36); kbindScroll.Position = U2(0,0,0,36)
  kbindScroll.BackgroundTransparency = 1; kbindScroll.ScrollBarThickness = 2
  kbindScroll.ScrollBarImageColor3 = C3(80,80,80)
  kbindScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
  kbindScroll.Visible = false; kbindScroll.ZIndex = 11
  local function setTab(tab)
    if tab == "menu" then
      ...
      tabKbindBtn.BackgroundColor3 = C3(20,20,20); tabKbindBtn.TextColor3 = C3(130,130,130)
    else
      ...
      tabMenuBtn.BackgroundColor3 = C3(20,20,20); tabMenuBtn.TextColor3 = C3(130,130,130)
    end
  end
  ...
  local function MakeSection(text)
    ...
    lbl.Size = U2(1,-20,0,16); lbl.Position = U2(0,12,0,yOff)
    lbl.BackgroundTransparency = 1; lbl.Text = text
    lbl.TextColor3 = C3(100,100,100); lbl.Font = FGOTB; lbl.TextSize = 9
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 11
    yOff = yOff + 18
  end
  local colIdx = 0
  local function MakeToggle2Col(labelText, currentState, onToggle)
    local col = colIdx % 2
    local W = 107
    local btn = IN("TextButton", menuScroll)
    btn.Size = U2(0,W,0,32); btn.Position = U2(0, 10+col*(W+6), 0, yOff)
    btn.BackgroundColor3 = currentState and C3(35,35,35) or C3(14,14,14)
    btn.BackgroundTransparency = currentState and 0.1 or 0.2
    btn.TextColor3 = currentState and C3(255,255,255) or C3(120,120,120)
    btn.Text = labelText.."  "..(currentState and "ON" or "OFF")
    btn.Font = FGOTB; btn.TextSize = 10; btn.ZIndex = 11
    IN("UICorner", btn).CornerRadius = U1(0,7); AddOutline(btn)
    colIdx = colIdx + 1
    if colIdx % 2 == 0 then yOff = yOff + 38 end
    local state = currentState
    btn.MouseButton1Click:Connect(function()
      state = not state
      btn.Text = labelText.."  "..(state and "ON" or "OFF")
      btn.TextColor3 = state and C3(255,255,255) or C3(120,120,120)
      btn.BackgroundColor3 = state and C3(35,35,35) or C3(14,14,14)
      btn.BackgroundTransparency = state and 0.1 or 0.2
      onToggle(state)
    end)
  end
  local function MakeInput(labelText, currentVal, onChanged)
    local lbl = IN("TextLabel", menuScroll)
    lbl.Size = U2(0,120,0,24); lbl.Position = U2(0,12,0,yOff)
    lbl.BackgroundTransparency = 1; lbl.Text = labelText
    lbl.TextColor3 = C3(160,160,160); lbl.Font = FGOTB; lbl.TextSize = 10
    lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 11
    local box = IN("TextBox", menuScroll)
    box.Size = U2(0,88,0,24); box.Position = U2(1,-100,0,yOff)
    box.BackgroundColor3 = C3(20,20,20); box.Text = tostring(currentVal)
    box.TextColor3 = C3(255,255,255); box.Font = FGOTB; box.TextSize = 10
    box.ZIndex = 11; box.ClearTextOnFocus = false
    IN("UICorner", box).CornerRadius = U1(0,5); AddOutline(box)
  ...
  MakeInput("Speed:", SETTINGS.TARGET_SPEED, function(v) SETTINGS.TARGET_SPEED = v end)
  MakeInput("Steal Speed:", SETTINGS.STEAL_SPEED, function(v) SETTINGS.STEAL_SPEED = v end)
  yOff = yOff + 4
  MakeSection("AUTO GRAB")
  MakeInput("Delay:", SETTINGS.GRAB_DELAY, function(v) SETTINGS.GRAB_DELAY = v end)
  MakeInput("Radius:", SETTINGS.GRAB_RADIUS, function(v) SETTINGS.GRAB_RADIUS = v end)
  yOff = yOff + 4
  MakeSection("MISC")
  colIdx = 0
  MakeToggle2Col("SPIN", SETTINGS.SPIN_ENABLED, function(on)
    SETTINGS.SPIN_ENABLED = on; if on then applySpin() else removeSpin() end
  end)
  MakeToggle2Col("UNWALK", SETTINGS.UNWALK, function(on) SETTINGS.UNWALK = on end)
  MakeToggle2Col("OPTIMIZER", false, function(on) if on then enableOptimizer() end end)
  MakeToggle2Col("XRAY", XrayBase.Enabled, function(on)
    if on then enableXrayBase() else disableXrayBase() end
  end)
  if colIdx % 2 ~= 0 then yOff = yOff + 38 end
  MakeInput("Spin Speed:", SETTINGS.SPIN_SPEED, function(v)
    SETTINGS.SPIN_SPEED = v; if SETTINGS.SPIN_ENABLED then applySpin() end
  end)
  yOff = yOff + 6
  local saveBtn = IN("TextButton", menuScroll)
  saveBtn.Size = U2(1,-20,0,28); saveBtn.Position = U2(0,10,0,yOff)
  saveBtn.BackgroundColor3 = C3(25,25,25); saveBtn.BackgroundTransparency = 0.1
  saveBtn.Text = "ðŸ’¾  SAVE"; saveBtn.TextColor3 = C3(180,180,180)
  saveBtn.Font = FGOTB; saveBtn.TextSize = 11; saveBtn.ZIndex = 11
  IN("UICorner", saveBtn).CornerRadius = U1(0,7); AddOutline(saveBtn)
  saveBtn.MouseButton1Click:Connect(function()
    saveMenuConfig(); saveKeybinds()
    saveBtn.Text = "âœ”  SAVED!"; saveBtn.TextColor3 = C3(100,220,100)
    task.delay(1.5, function()
      saveBtn.Text = "ðŸ’¾  SAVE"; saveBtn.TextColor3 = C3(180,180,180)
    end)
  end)
  ...
  local function refreshKbindColors()
    for action, btn in pairs(actionBtns) do
      local isSel = (selectedAction == action)
      btn.BackgroundColor3 = isSel and C3(70,30,150) or C3(18,18,18)
      btn.TextColor3 = isSel and C3(255,255,255) or C3(170,170,170)
      btn.BackgroundTransparency = isSel and 0.0 or 0.3
    end
    for keyName, btn in pairs(keyBtns) do
      ...
      btn.BackgroundColor3 = isBound and C3(0,150,55) or C3(18,18,18)
      btn.TextColor3 = isBound and C3(255,255,255) or C3(150,150,150)
      btn.BackgroundTransparency = isBound and 0.0 or 0.4
    end
  end
  local hint = IN("TextLabel", kbindScroll)
  hint.Size = U2(1,-20,0,14); hint.Position = U2(0,10,0,6)
  hint.BackgroundTransparency = 1; hint.Text = "â‘  azione  â†’  â‘¡ tasto"
  hint.TextColor3 = C3(90,90,90); hint.Font = FGOTB; hint.TextSize = 9; hint.ZIndex = 11
  local kyOff = 24
  local AW = 107; local AH = 28
  for i, action in ipairs(KBIND_ACTIONS) do
    local col = (i-1) % 2; local row = math.floor((i-1)/2)
    local btn = IN("TextButton", kbindScroll)
    btn.Size = U2(0,AW,0,AH); btn.Position = U2(0, 10+col*(AW+6), 0, kyOff+row*(AH+4))
    btn.BackgroundColor3 = C3(18,18,18); btn.BackgroundTransparency = 0.3
    btn.Text = action.."  ["..(KEYBINDS[action] or "-").."]"
    btn.TextColor3 = C3(170,170,170); btn.Font = FGOTB; btn.TextSize = 9; btn.ZIndex = 11
    IN("UICorner", btn).CornerRadius = U1(0,6); AddOutline(btn)
  ...
  kyOff = kyOff + math.ceil(#KBIND_ACTIONS/2)*(AH+4) + 6
  local sepK = IN("Frame", kbindScroll)
  sepK.Size = U2(1,-20,0,1); sepK.Position = U2(0,10,0,kyOff)
  sepK.BackgroundColor3 = C3(45,45,45); sepK.BorderSizePixel = 0; sepK.ZIndex = 11
  kyOff = kyOff + 6
  local COLS_K = 6
  local KW = math.floor((250-20)/COLS_K) - 2
  for i, keyName in ipairs(ALL_KEYS) do
    local col = (i-1) % COLS_K; local row = math.floor((i-1)/COLS_K)
    local label = keyName:gsub("GP:Button",""):gsub("GP:DPad",""):gsub("GP:","")
    if #label > 4 then label = label:sub(1,4) end
    local btn = IN("TextButton", kbindScroll)
    btn.Size = U2(0,KW,0,KW); btn.Position = U2(0, 10+col*(KW+2), 0, kyOff+row*(KW+2))
    btn.BackgroundColor3 = C3(18,18,18); btn.BackgroundTransparency = 0.4
    btn.Text = label; btn.TextColor3 = C3(150,150,150)
    ...
    btn.MouseButton1Click:Connect(function()
      ...
      if actionBtns[selectedAction] then
        actionBtns[selectedAction].Text = selectedAction.."  ["..keyName.."]"
      end
      selectedAction = nil; refreshKbindColors(); saveKeybinds()
    end)
  end
  refreshKbindColors()
end
