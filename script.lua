...
if isAdmin() then
    ...
    adminBtn.BackgroundColor3 = Color3.fromRGB(60, 20, 120)
    ...
    adminStroke.Color = Color3.fromRGB(160, 80, 255)
    ...
    adminBtn.MouseButton1Click:Connect(function()
        ...
        ps.Color = Color3.fromRGB(130, 60, 220)
        ps.Thickness = 2
...
kbs.Color = Color3.fromRGB(100, 50, 180)
kbs.Thickness = 1.5
...
local function startStealLoop(prompt)
    ...
    task.spawn(function()
        ...
        currentTween = TweenService:Create(ProgressBarFill, TweenInfo.new(0.2, Enum.EasingStyle.Linear), {Size = UDim2.new(1,0,1,0)})
        currentTween:Play(); task.wait(0.2)
        ...
        fireproximityprompt(prompt); task.wait(0.05); fireproximityprompt(prompt); resetBar()
    end)
end
...
TauntBtn.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
...
MakeDraggable(TauntBtn, function()
    ...
    if tauntActive then
        ...
        TauntBtn.BackgroundColor3 = Color3.fromRGB(130, 80, 255)
    ...
    if SETTINGS.ENABLED and not isStealing then
        ...
        for _, descendant in pairs(workspace:GetDescendants()) do
            if descendant:IsA("ProximityPrompt") and descendant.Enabled and descendant.ActionText == "Steal" then
                ...
                if pos then local distance = (hrp.Position - pos).Magnitude; if distance <= minDist then minDist = distance; nearestPrompt = descendant end end
            end
        end
