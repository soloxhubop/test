...
local function launchMainScript()
  ...
  InfoLabel.Font = FGOT
  InfoLabel.TextSize = math.floor(9*scale)
  InfoLabel.ZIndex = 6
  ...
  local SAVE_KEYS = {
    ...
    "STEAL_DURATION", "LOCK_SPEED", "JUMP_FORCE",
    "SPIN_ENABLED", "SPIN_SPEED", "UNWALK"
  }
  ...
  loadConfig()
  SETTINGS.TARGET_SPEED = 58.5
  SETTINGS.STEAL_SPEED = 29
    ...
    meleeConn=RunService.Heartbeat:Connect(function()
      ...
      if target and torso then
        if Cebo.Align then
          ...
          Cebo.Align.CFrame=CFrame.lookAt(h.Position,V3(target.Position.X,h.Position.Y,target.Position.Z))
          hum.AutoRotate=false
        end
        local fd=V3(torso.Position.X-h.Position.X,0,torso.Position.Z-h.Position.Z)
        if fd.Magnitude>1.5 then
          local u=fd.Unit
          h.AssemblyLinearVelocity=V3(u.X*SETTINGS.LOCK_SPEED,h.AssemblyLinearVelocity.Y,u.Z*SETTINGS.LOCK_SPEED)
        else
  ...
  local function executeInternalSteal(prompt)
    ...
    task.spawn(function()
      ...
      task.wait(0.01)
      data.ready = true
      -- isStealing rimane true finchÃ© i waypoint non finiscono
    end)
    return true
  end
  ...
  -- Scala automatica in base al dispositivo

  local function MakeBtn(parent,text,bx,by,bw,bh)
    local btn=IN("TextButton",parent)
    btn.Size=U2(0,bw,0,bh) btn.Position=U2(0,bx,0,by)
    btn.BackgroundColor3=C_OFF btn.BackgroundTransparency=0.1
    btn.Text="" btn.ZIndex=5 btn.AutoButtonColor=false
    IN("UICorner",btn).CornerRadius=U1(0,12)
    local s=IN("UIStroke",btn) s.Color=BOFF s.Thickness=1.5
    s.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    local dot=IN("Frame",btn)
    dot.Size=U2(0,8,0,8) dot.Position=U2(0,10,0.5,-4)
    dot.BackgroundColor3=DOFF dot.ZIndex=6
    IN("UICorner",dot).CornerRadius=U1(0.5,0)
    local lbl=IN("TextLabel",btn)
    lbl.Size=U2(1,-26,1,0) lbl.Position=U2(0,22,0,0)
    lbl.BackgroundTransparency=1 lbl.Text=text
    lbl.TextColor3=TOFF lbl.Font=FGOTB lbl.TextSize=BTEXT
    lbl.TextXAlignment=Enum.TextXAlignment.Left lbl.ZIndex=6
    return btn,dot,lbl,s
  end
  local function SetActive(dot,lbl,s,on)
    dot.BackgroundColor3=on and DON or DOFF
    lbl.TextColor3=on and TON or TOFF
    s.Color=on and BON or BOFF
  end
  local SaveBtn=IN("TextButton",HUDScreen)
  SaveBtn.Size=U2(0,70,0,28) SaveBtn.Position=U2(0,LX,0,24)
  SaveBtn.BackgroundColor3=C_OFF SaveBtn.BackgroundTransparency=0.1
  SaveBtn.Text="SAVE" SaveBtn.TextColor3=TOFF
  SaveBtn.Font=FGOTB SaveBtn.TextSize=math.floor(11*scale) SaveBtn.ZIndex=5 SaveBtn.AutoButtonColor=false
  IN("UICorner",SaveBtn).CornerRadius=U1(0,10)
  ...
  MakeDraggable(agBtn,function() setAutoGrabState(not agOn) end)
  ...
  MakeDraggable(lkBtn,function() setLockState(not lkOn) end)
  ...
  MakeDraggable(flBtn,function() setFloatState(not flOn) end)
  ...
  -- funzioni interne senza bottoni visibili
  local alOn=false local arOn=false
  local alDot,alLbl,alStroke={},{},{}
  local arDot,arLbl,arStroke={},{},{}
  local function setAutoLeftState(on)
    alOn=on SETTINGS.AUTOLEFT=on
    if on then SETTINGS.AUTORIGHT=false; arOn=false; stopMovement(); startMovement("left")
    else stopMovement() end
  end
  local function setAutoRightState(on)
    arOn=on SETTINGS.AUTORIGHT=on
    if on then SETTINGS.AUTOLEFT=false; alOn=false; stopMovement(); startMovement("right")
    else stopMovement() end
  end
  local apBtn2,apDot2,apLbl2,apStroke2=MakeBtn(HUDScreen,"AUTO PLAY",0,0,BW,BH)
  apBtn2.Position=U2(1,RX,0,Y3) allButtons["AutoPlay"]=apBtn2 local apOn2=false
  local function setAutoPlayState2(on)
    apOn2=on SetActive(apDot2,apLbl2,apStroke2,on)
    if on then
      -- Rileva lato base del giocatore e vai al lato opposto
      local char=LocalPlayer.Character
      local hrp=char and char:FindFirstChild("HumanoidRootPart")
      if not hrp then return end
      local myBaseIsLeft=false local found=false
      local plots=workspace:FindFirstChild("Plots")
      if plots then
        for _,pl in ipairs(plots:GetChildren()) do
          local sign=pl:FindFirstChild("PlotSign")
          local yb=sign and sign:FindFirstChild("YourBase")
          if yb and yb.Enabled then
            -- La base Ã¨ a sinistra se Z > 60, a destra se Z < 60
            local podiums=pl:FindFirstChild("AnimalPodiums")
            if podiums then
              local pod=podiums:FindFirstChildOfClass("Model")
              if pod then
                myBaseIsLeft = pod:GetPivot().Position.Z > 60
                found=true
              end
            end
          end
      ...
      if not found then
        -- fallback: usa posizione HRP
        myBaseIsLeft = hrp.Position.Z > 60
      end
      -- Base a sinistra (Z>60) -> vai a destra, base a destra (Z<60) -> vai a sinistra
      if myBaseIsLeft then
        setAutoRightState(true)
        setAutoLeftState(false)
      else
        setAutoLeftState(true)
  ...
  MakeDraggable(apBtn2,function() setAutoPlayState2(not apOn2) end)
  ...
  local function buildMenuPanel()
    ...
    panel.Size=U2(0,220,0,10) panel.Position=U2(0.5,-110,0.5,-145)
    panel.BackgroundColor3=C_OFF panel.BackgroundTransparency=0.05 panel.ZIndex=10
    IN("UICorner",panel).CornerRadius=U1(0,14)
    local pS=IN("UIStroke",panel) pS.Color=C3(120,40,220) pS.Thickness=1.5
    local pT=IN("TextLabel",panel)
    pT.Size=U2(1,0,0,32) pT.BackgroundTransparency=1 pT.Text="MELOSKA MENU"
    pT.Font=FGOTB pT.TextSize=13 pT.ZIndex=11
    pT.TextColor3=C3(255,255,255)
    local xBtn=IN("TextButton",panel)
    xBtn.Size=U2(0,26,0,26) xBtn.Position=U2(1,-30,0,3)
    xBtn.BackgroundColor3=C3(40,20,20) xBtn.Text="X" xBtn.TextColor3=C3(220,80,80)
    xBtn.Font=FGOTB xBtn.TextSize=12 xBtn.ZIndex=12 xBtn.AutoButtonColor=false
    IN("UICorner",xBtn).CornerRadius=U1(0.5,0)
    xBtn.MouseButton1Click:Connect(function()
      menuOpen=false panelGui:Destroy(); menuPanel=nil; SetActive(mnDot,mnLbl,mnStroke,false)
    end)
    MakeDraggable(panel,nil)
    local yO=38
    local function MT(lt,cs,cb)
      local b=IN("TextButton",panel)
      b.Size=U2(1,-20,0,38) b.Position=U2(0,10,0,yO)
      b.BackgroundColor3=cs and C_ON or C_OFF b.BackgroundTransparency=0.1
      b.Text=lt.."\n"..(cs and "ON" or "OFF") b.TextColor3=cs and TON or TOFF
      b.Font=FGOTB b.TextSize=11 b.ZIndex=11 b.AutoButtonColor=false
      IN("UICorner",b).CornerRadius=U1(0,8)
      local bS=IN("UIStroke",b) bS.Color=cs and BON or BOFF bS.Thickness=1.5
      yO=yO+46 local st=cs
      b.MouseButton1Click:Connect(function()
        st=not st b.Text=lt.."\n"..(st and "ON" or "OFF")
        b.TextColor3=st and TON or TOFF b.BackgroundColor3=st and C_ON or C_OFF
        bS.Color=st and BON or BOFF cb(st)
      end)
    end
    local function IR(lt,cv,cb)
      local il=IN("TextLabel",panel)
      il.Size=U2(0,100,0,28) il.Position=U2(0,10,0,yO)
      il.BackgroundTransparency=1 il.Text=lt il.TextColor3=TOFF
      il.Font=FGOTB il.TextSize=11 il.ZIndex=11
      il.TextXAlignment=Enum.TextXAlignment.Left
      local bx=IN("TextBox",panel)
      bx.Size=U2(0,90,0,28) bx.Position=U2(1,-100,0,yO)
      bx.BackgroundColor3=C3(25,25,25) bx.Text=tostring(cv)
      bx.TextColor3=TON bx.Font=FGOTB bx.TextSize=11 bx.ZIndex=11
    ...
    IR("Speed:",SETTINGS.TARGET_SPEED,function(v) SETTINGS.TARGET_SPEED=v end)
    IR("Steal Speed:",SETTINGS.STEAL_SPEED,function(v) SETTINGS.STEAL_SPEED=v end)
    MT("SPIN",SETTINGS.SPIN_ENABLED,function(on) SETTINGS.SPIN_ENABLED=on; if on then applySpin() else removeSpin() end end)
    IR("Spin Speed:",SETTINGS.SPIN_SPEED,function(v) SETTINGS.SPIN_SPEED=v; if SETTINGS.SPIN_ENABLED then applySpin() end end)
    MT("UNWALK",SETTINGS.UNWALK,function(on) SETTINGS.UNWALK=on end)
    -- KEYBINDS
    local kbTitle=IN("TextLabel",panel)
    kbTitle.Size=U2(1,-20,0,20) kbTitle.Position=U2(0,10,0,yO)
    kbTitle.BackgroundTransparency=1 kbTitle.Text="âŒ¨ Keybinds (RMB = rimuovi)"
    kbTitle.TextColor3=DON kbTitle.Font=FGOTB kbTitle.TextSize=10
    kbTitle.TextXAlignment=Enum.TextXAlignment.Left kbTitle.ZIndex=11
    yO=yO+24
    local kbList={
      {"Grab","KEYBIND_GRAB"},{"Bat Lock","KEYBIND_LOCK"},
      {"Auto Left","KEYBIND_LEFT"},{"Auto Right","KEYBIND_RIGHT"},
      {"Auto Play","KEYBIND_PLAY"},{"TP Auto","KEYBIND_TP"},{"Float","KEYBIND_FLOAT"}
    }
    for _,kb in ipairs(kbList) do
      local kl=IN("TextLabel",panel)
      kl.Size=U2(0,85,0,24) kl.Position=U2(0,10,0,yO)
      kl.BackgroundTransparency=1 kl.Text=kb[1]
      kl.TextColor3=TOFF kl.Font=FGOTB kl.TextSize=10
      kl.TextXAlignment=Enum.TextXAlignment.Left kl.ZIndex=11
      local kb2=IN("TextButton",panel)
      kb2.Size=U2(0,95,0,24) kb2.Position=U2(1,-105,0,yO)
      kb2.BackgroundColor3=C3(25,25,25) kb2.BackgroundTransparency=0.1
      kb2.Text=SETTINGS[kb[2]] kb2.TextColor3=SETTINGS[kb[2]]~="None" and DON or TOFF
      kb2.Font=FGOTB kb2.TextSize=10 kb2.ZIndex=11 kb2.AutoButtonColor=false
      IN("UICorner",kb2).CornerRadius=U1(0,6)
      IN("UIStroke",kb2).Color=BOFF
      local waiting=false
      kb2.MouseButton1Click:Connect(function()
        if waiting then return end
        waiting=true kb2.Text="..." kb2.TextColor3=DON
        local conn conn=UserInputService.InputBegan:Connect(function(inp,gpe)
          if gpe then return end
          if inp.UserInputType==Enum.UserInputType.Keyboard then
            SETTINGS[kb[2]]=inp.KeyCode.Name
            kb2.Text=inp.KeyCode.Name kb2.TextColor3=DON
          else
    ...
    local svB=IN("TextButton",panel)
    svB.Size=U2(1,-20,0,34) svB.Position=U2(0,10,0,yO)
    svB.BackgroundColor3=C3(80,30,160) svB.BackgroundTransparency=0.1
    svB.Text="SAVE CONFIG" svB.TextColor3=TON svB.Font=FGOTB
    svB.TextSize=11 svB.ZIndex=11 svB.AutoButtonColor=false
    IN("UICorner",svB).CornerRadius=U1(0,8)
    ...
    yO=yO+42 panel.Size=U2(0,220,0,yO+12)
  end
  MakeDraggable(mnBtn,function()
  ...
  UserInputService.InputBegan:Connect(function(inp,gpe)
    ...
    if SETTINGS.KEYBIND_PLAY~="None" and k==SETTINGS.KEYBIND_PLAY  then setAutoPlayState2(not apOn2) end
  ...
  if apOn2 then setAutoPlayState2(true) end
