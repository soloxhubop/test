...
local function generateKey()
    ...
    return "MELOSKA-" .. seg(4) .. "-" .. seg(4) .. "-" .. seg(4)
end
...
-- responsive: si adatta a mobile e PC
local _vp = workspace.CurrentCamera.ViewportSize
local _isMobile = _vp.X < 700
local _winW = _isMobile and math.min(_vp.X * 0.92, 420) or 460
local _winH = _isMobile and math.min(_vp.Y * 0.88, 530) or 530
local _scale = _winH / 530 -- scala tutto proporzionalmente
...
topBar.Size = UDim2.new(1, 0, 0, math.floor(50*_scale))
...
local closeHeaderBtn = makeHeaderBtn("Ã¢Å“â€¢", -10, Color3.fromRGB(200, 50, 50))
makeHeaderBtn("Ã¢Ëœâ€¦", -46, Color3.fromRGB(60, 60, 60))
...
local _logoSz = math.floor(56*_scale)
logoCircle.Size = UDim2.new(0, _logoSz, 0, _logoSz)
logoCircle.Position = UDim2.new(0.5, -_logoSz/2, 0, math.floor(62*_scale))
...
productLbl.Size = UDim2.new(1, 0, 0, math.floor(30*_scale))
productLbl.Position = UDim2.new(0, 0, 0, math.floor(128*_scale))
...
productLbl.TextSize = math.floor(18*_scale)
productLbl.ZIndex = 12

local dotFrame = Instance.new("Frame", win)
dotFrame.Size = UDim2.new(0, 140, 0, 18)
dotFrame.Position = UDim2.new(0.5, -70, 0, math.floor(164*_scale))
...
sep1.Position = UDim2.new(0, 20, 0, math.floor(196*_scale))
...
keyBox.Size = UDim2.new(1, -40, 0, math.floor(46*_scale))
keyBox.Position = UDim2.new(0, 20, 0, math.floor(210*_scale))
...
redeemBtn.Size = UDim2.new(1, -40, 0, math.floor(50*_scale))
redeemBtn.Position = UDim2.new(0, 20, 0, math.floor(268*_scale))
redeemBtn.BackgroundColor3 = Color3.fromRGB(110, 40, 220)
redeemBtn.BorderSizePixel = 0
redeemBtn.Text = "Ã¢â‚¬Âº Redeem"
...
statusLbl.Position = UDim2.new(0, 20, 0, math.floor(326*_scale))
...
sep2.Position = UDim2.new(0, 20, 0, math.floor(356*_scale))
...
supportLbl.Position = UDim2.new(0, 0, 0, math.floor(364*_scale))
supportLbl.BackgroundTransparency = 1
supportLbl.Text = "Ã¢â‚¬Â¢ Need support?  Join Discord"
...
local function makeSocialBtn(text, xPos)
    ...
    local _btnW = math.floor((_winW - 60) / 3)
    local _xReal = 20 + (xPos == 20 and 0 or (xPos == 160 and _btnW + 10 or (_btnW + 10)*2))
    btn.Size = UDim2.new(0, _btnW, 0, math.floor(42*_scale))
    btn.Position = UDim2.new(0, _xReal, 0, math.floor(394*_scale))
...
getKeyLbl.Position = UDim2.new(0, 0, 0, math.floor(448*_scale))
getKeyLbl.BackgroundTransparency = 1
getKeyLbl.Text = "Ã¢â‚¬Â¢ Ã¢â‚¬Â¢  Click a button below to get your key"
...
local _bjSz = math.floor(46*_scale)
bottomJ.Size = UDim2.new(0, _bjSz, 0, _bjSz)
bottomJ.Position = UDim2.new(0.5, -_bjSz/2, 0, math.floor(474*_scale))
...
if _LocalPlayer.UserId == ADMIN_ID then
    ...
    adminBtn.Text = "Ã¢Å¡Â¡ Genera Key (Admin)"
...
local function tryRedeem(inputKey)
    ...
    if valid then
        ...
        redeemBtn.Text = "Ã¢â‚¬Âº Redeem"
        redeemBtn.BackgroundColor3 = Color3.fromRGB(110, 40, 220)
        statusLbl.Text = "Ã¢Å“Ëœ Key non valida o scaduta!"
